...
const Navbar = () => {
  ...
  const sections = ['about', 'experience', 'projects', 'skills', 'building', 'contact']
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-slate-900/95 backdrop-blur-2xl border-b border-slate-800/50 shadow-2xl">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex justify-between items-center py-5">
          <Link 
            to="hero" 
            className="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-white via-slate-100 to-slate-200 bg-clip-text text-transparent hover:scale-[1.02] transition-all duration-300"
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {sections.map((section) => (
              ...
              >
                {section === 'building' ? 'Building' : section.charAt(0).toUpperCase() + section.slice(1)}
              </Link>
            ))}
            <a 
            ...
            <div className="flex flex-col space-y-3 pt-4 border-t border-slate-800">
              {sections.map((section) => (
                <Link
                  ...
                  className={`py-3 px-4 font-semibold rounded-xl transition-colors hover:bg-slate-800/50 ${
                    activeSection === section ? 'bg-accent/20 text-accent border border-accent/30' : 'text-slate-300 hover:text-white'
                  }`}
                >
                  {section === 'building' ? 'Building' : section.charAt(0).toUpperCase() + section.slice(1)}
                </Link>
